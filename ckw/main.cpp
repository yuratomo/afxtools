...
#ifdef __AFXCKW__
HWND     gParentHwnd = NULL;
HWND     ghWndLastFocus = NULL;
bool     gIMEstart = false;
int      *gsmode = NULL;
HANDLE   ghShare = NULL;
HANDLE   ghSharedBuffer = NULL;
char     *gSharedBufer = NULL;
COLORREF gColorTableNotFocus = RGB(0,255,0);
#endif // __AFXCKW__
...
#define AFXCKW_SHAREDMEM_NAME  L"__afxckw_shared_memory__"
#define CKW_SHAREDBUFFER_NAME  L"__ckw_shared_buffer__"
#define FOCUS_AFX 0
#define FOCUS_CKW 1
#endif
...
#ifdef __AFXCKW__
	if (GetFocus() == hWnd && gsmode != NULL && gsmode[1] == FOCUS_CKW) {
		HPEN pen = CreatePen(PS_SOLID, 2, gColorTableNotFocus);
		HGDIOBJ oldPen   = SelectObject(hMemDC, pen);
		HGDIOBJ oldBrush = SelectObject(hMemDC, gBgBrush);
		Rectangle(hMemDC, rc.left+1, rc.top+1, rc.right-1, rc.bottom-1);
...
#ifdef __AFXCKW__
	// カレント行の一つ前の行を共有メモリに書き出す
	if (gSharedBufer != NULL && gCSI->dwCursorPosition.Y > 1) {
		int pntX = 0;
		int pntY = (gCSI->dwCursorPosition.Y - 1) - gCSI->srWindow.Top;
		ptr = gScreen + CSI_WndCols(gCSI) * pntY + pntX;

		int	x, idx=0;
		//wchar_t* work_text = (wchar_t*)gSharedBufer;
		char* work_text = (char*)gSharedBufer;
		memset(gSharedBufer, 0, 4096);
		for(x = gCSI->srWindow.Left ; x <= gCSI->srWindow.Right && idx<4096 ; x++, idx++) {
			...
			//*work_text++ = ptr->Char.UnicodeChar;
			*work_text++ = ptr->Char.AsciiChar;
			ptr++;
		}
	}
#endif // __AFXCKW__
...
void SetFocusOtherProcess(HWND hWndDst)
{
	...
	::AttachThreadInput(::GetCurrentThreadId(), tidDst, TRUE);
	::SetActiveWindow(hWndDst);
	::SetForegroundWindow(hWndDst);
	::SetFocus(hWndDst);
	::AttachThreadInput(::GetCurrentThreadId(), tidDst, FALSE);
}
#endif // __AFXCKW__
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_SYSKEYUP:
		...
		if(wp == 'Q') {
			if (gParentHwnd != NULL) {
				SetFocusOtherProcess(gParentHwnd);
				if (gsmode != NULL) {
					gsmode[1] = FOCUS_AFX;
				}
				InvalidateRect(hWnd, NULL, TRUE);
	...
	case WM_KEYUP:
		if((wp == VK_NEXT || wp == VK_PRIOR ||
		   ...
		   (GetKeyState(VK_SHIFT) & 0x8000)) {
			...
			if (gParentHwnd != NULL && msg == WM_KEYUP) {
				SetFocusOtherProcess(gParentHwnd);
				if (gsmode != NULL) {
					gsmode[1] = FOCUS_AFX;
				}
...
#ifdef __AFXCKW__
	case WM_COPYDATA:
		pCD = (COPYDATASTRUCT*)lp;
		if (pCD != NULL) {
			char* _text = (char*)pCD->lpData;
			int _len = pCD->cbData;
			if (_len <= 4096) {
				wchar_t* text = new wchar_t[ _len ];
				int len = sizeof(wchar_t) * (_len);
				ZeroMemory(text, len);
				MultiByteToWideChar(CP_ACP, 0, _text, -1, text, len );
				writeConsole(text, _len-1);
				writeConsoleReturn();
				delete[] text;
			}
		}
...
static BOOL create_window(ckOpt& opt)
{
...
#ifdef __AFXCKW__
	gParentHwnd = NULL;
	gParentHwnd = (HWND)opt.getParentHwnd();
	if (gParentHwnd != NULL) {
		className = L"CkwAfxWindowClass";
		ghShare = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE,
				0, 8, AFXCKW_SHAREDMEM_NAME);
		if (ghShare != NULL) {
			gsmode = (int*)::MapViewOfFile(ghShare, FILE_MAP_WRITE, 0, 0, 4);
		}
	}
	ghSharedBuffer = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE,
			0, 4096, CKW_SHAREDBUFFER_NAME);
	if (ghSharedBuffer != NULL) {
		gSharedBufer = (char*)::MapViewOfFile(ghSharedBuffer, FILE_MAP_WRITE, 0, 0, 4096);
	}
#endif // __AFXCKW__
...
#ifdef __AFXCKW__
	if (gParentHwnd != NULL) {
		gFrame.left += GetSystemMetrics(SM_CXBORDER);
		gFrame.right -= GetSystemMetrics(SM_CXBORDER);
		gFrame.top += GetSystemMetrics(SM_CYCAPTION);
		gFrame.top += GetSystemMetrics(SM_CYBORDER);
		gFrame.bottom -= GetSystemMetrics(SM_CYBORDER);
	}
#endif // __AFXCKW__
	...
	if (gParentHwnd != NULL) {
		...
		//width = 1;
		//height = 1;
		ghWndLastFocus = ::GetFocus();
	}
	HWND hWnd = CreateWindowEx(exstyle, className, title, style,
				   posx, posy, width, height,
				   NULL, NULL, hInstance, NULL);
	if(!hWnd && !( !conf_title || !conf_title[0]) ){
		delete [] title;
		return(FALSE);
    }
	if (gParentHwnd != NULL) {
		::SetWindowLong(hWnd, GWL_STYLE, WS_CHILD);
		::SetParent(hWnd, gParentHwnd);
	}
#else
	HWND hWnd = CreateWindowEx(exstyle, className, title, style,
				   posx, posy, width, height,
...
#ifdef __AFXCKW__
	gColorTableNotFocus = opt.getColorFocus();
#endif // __AFXCKW__
...
#ifdef __AFXCKW__
	if (ghShare != NULL) {
		UnmapViewOfFile(gsmode);
		CloseHandle(ghShare);
	}
	if (ghSharedBuffer != NULL) {
		UnmapViewOfFile(gSharedBufer);
		CloseHandle(ghSharedBuffer);
	}
#endif // __AFXCKW__
}
